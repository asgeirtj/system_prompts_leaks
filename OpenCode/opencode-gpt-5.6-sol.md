...
prompt as assembled at runtime. Reproduce with `node capture/capture.mjs opencode --model gpt-5.6-sol`. opencode `run` mode, model `gpt-5.6-sol` through a self-hosted gateway, 10,413 characters and 9 tools as sent. Home directory, username, git identity, gateway host, OS build,
...
[prompt/OPENCODE/gpt-5-6-sol-system-prompt.md](https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/OPENCODE/gpt-5-6-sol-system-prompt.md).</sub>
