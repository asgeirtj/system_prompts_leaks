...
<sub>Captured 2 September 2026 off the wire with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) (Apache-2.0), not extracted from the
shipped bundle: a local proxy in front of the model API recorded the request the harness actually sent, so this is the
prompt as assembled at runtime. Reproduce with `node capture/capture.mjs opencode --model muse-spark-1.2-contributor-free`. opencode `run` mode, model `muse-spark-1.2-contributor-free`, 10,249 characters and 11 tools as sent. Home directory, username, git identity, gateway host, OS build,
uuids and long hex runs are replaced with `{PLACEHOLDER}` tokens; nothing else is reworded or reordered. Tool
definitions travelled with the same request and are not included in this file. Public copy:
[prompt/OPENCODE/muse-spark-1-2-contributor-free-system-prompt.md](https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/OPENCODE/muse-spark-1-2-contributor-free-system-prompt.md).</sub>
